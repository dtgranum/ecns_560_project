...
uniquedonationsPSID <- unique(donationsPSID)
...
cleandonationsPSID <- donationsPSIDtreatpost |>
...
ols_fe_donations <- feols(donations ~ income | year + state, data = cleandonationsPSID)
...
ols_fe_attendance <- feols(attendance ~ treatpost | year + family, data = cleanattendancePSID)
etable(ols_fe_donations, ols_fe_donations)
```
...
cleanaffiliationPSID <- affiliationPSID |>
  filter(!religious_pref == 99) |>
  filter(!is.na(state)) |>
  filter(!state == 99) |>
  filter(!state == 0)

ols_fe_affiliation <- feols(affiliation ~ treatpost | year + family, data = cleanaffiliationPSID)
etable(ols_fe_donations, ols_fe_donations)
```
