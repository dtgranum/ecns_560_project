...
```
Our analysis is most likely not causal. While we implemented a natural experiment there are several flaws such as a lack of control variables, it is unclear what mechanisms are at work, and using a country-wide analysis. For example, political affiliation is an omitted variable that is positively correlated with religious attendance and negatively correlated with Medicaid expansion being passed so this would cause a downward bias. There is a lot of measurement error in survey data which would cause a downward bias.

Say a few words about how you might proceed from here, if you were going to more convincingly answer your causal research question.

Proceeding from here we would do more work into investigating if Medicaid expansion is a good natural experiment to implement through an event study.
We would also look at a more specific region of the country rather than the whole country since that causes a lot of noise in the data. For example we would compare Wisconsin-Minnesota or Alabama-Louisiana.
Proceeding from here we would change our treatment and comparison groups so that the treatment group are people who now qualify for Medicaid expansion while the comparison group are people who would have otherwise qualified for Medicaid expansion but live in a state that did not accept Medicaid expansion. We also see that when we restrict the sample to just families with lower income levels our estimate becomes larger which indicates to us that this would be a good approach to take.
